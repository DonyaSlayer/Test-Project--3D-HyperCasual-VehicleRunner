...
public class EnemySpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            _poolManager.CreatePool("EnemyVFX", _enemyVfxPrefab, 20);
        }
        if (_damageTextPrefab != null)
        {
            _poolManager.CreatePool("DamageText", _damageTextPrefab, 30);
        }
        if (_coinTextPrefab != null)
        {
            _poolManager.CreatePool("CoinText", _coinTextPrefab, 20);
        }
    }
    public void SpawnEnemiesOnChunk(Vector3 chunkCenter, int currentChunkIndex)
    {
        ...
        for(int i = 0; i < enemiesToSpawn; i++)
        {
            ...
            if (enemyObj != null)
            {
                ...
                if (enemy != null)
                {
                    enemy.Initiialize(_playerTransform, _poolManager, _gameStateController, _coinManager);
                }
            }
        }
    }
