...
public class Enemy : MonoBehaviour, IDamageable
{
    ...
    public void Initiialize(Transform player, ObjectPoolManager poolManager, GameStateController gameStateController, CoinManager coinManager)
    {
    ...
    {
        HandlePushBack();
        if (_gameStateController == null || _gameStateController.CurrentState != GameState.Playing) return;
    ...
    {
        if (distance <= _detectionRadius && !_isAlerting && _cts != null)
        {
            _isAlerting = true;
    ...
    private void Die()
    {
        ...
        CancelTasks();
        int coinsToDrop = Random.Range(5, 11);
        if(_coinManager != null) _coinManager.AddCoinsForRun(coinsToDrop);
        SpawnCoinText(coinsToDrop);
        GameObject vfx = _poolManager.SpawnFromPool("EnemyVFX", transform.position, Quaternion.identity);
        if(vfx != null)
        {
            vfx.GetComponent<VfxPoolReturner>()?.Init(_poolManager, "EnemyVFX", 5f);
    ...
    private void SpawnCoinText(int amount)
    {
        ...
        GameObject textObj = _poolManager.SpawnFromPool("CoinText", _coinTextSpawnPoint.position + Vector3.up * 0.5f, Quaternion.identity);
        {
            UIFloatingText floatingText = textObj.GetComponent <UIFloatingText>();
